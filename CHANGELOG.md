...
## [Release v3.5.2] — 2026-08-17 (Фиксация статуса закрытия смены и кнопка создания заявки)

### Изменено
- **Панель управления маршрутом (`result.tsx`)**:
  - Удалена кнопка «Построить заново» в шапке и мобильном меню; на её месте оставлена только кнопка **«Создать новую заявку»** (`/orders`).
  - Исправлено сохранение состояния кнопки закрытия смены: после нажатия «Закрыть смену» кнопка переходит в постоянный статус **«Смена закрыта»** и сохраняет его при повторных заходах и обновлении страницы.
- **Интерфейс водителя (`driver.tsx`)**:
  - Добавлена реактивная синхронизация статуса закрытия смены с бэкендом: если рейс закрыт диспетчером, карточка и ведомость водителя автоматически фиксируются в состоянии закрытой смены.
- **Бэкенд API (`main.py`)**:
  - Эндпоинты `GET /api/route/sessions/{id}` и `GET /api/route/sessions/{id}/assignments` теперь возвращают флаги `is_completed` и время закрытия `completed_at`.
  - Улучшен текст благодарности водителю в Telegram без лишних ссылок на скачивание и с контактом диспетчера при необходимости.

---

## [Release v3.5.1] — 2026-08-17 (Оптимизация шапки результатов, авто-ширина колонок Excel и благодарность водителю в Telegram)

### Изменено
- **Шапка страницы результатов (`result.tsx`)**:
  - Убрана лишняя кнопка выгрузки Excel из верхнего заголовка страницы результатов (выгрузка Excel доступна во вкладке «Исполнение доставок»).
  - Кнопка **«Построить заново»** возвращена в правый верхний блок действий.
  - Добавлена кнопка **«Создать новую заявку»**, ведущая напрямую на страницу создания заявок (`/orders`).
  - Убран нижний дублирующий блок «Построить заново».
  - В карточках водителей убрана дублирующая зелёная кнопка PDF (оставлена лаконичная кнопка «Отчёт PDF»).
- **Авто-форматирование Excel-отчёта (`report.xlsx`)**:
  - На листе **«Сводка по рейсу»** ширина колонки A увеличена до 48 символов, что предотвращает обрезание названий KPI и избавляет от необходимости вручную растягивать столбцы.
  - Добавлен отдельный показатель и пояснительное примечание для заказов без фиксированной суммы (расчёт и сверка по бумажной накладной).
  - На листе **«Детализация точек»** внедрена авто-подгонка ширины столбцов под длину текста с минимальными порогами читаемости; для точек без фиксированной цены выводится статус «По накладной».
- **Telegram-уведомление водителю**:
  - При завершении смены отправляется тёплое сообщение с благодарностью за безопасный рейс и хорошую работу без лишних ссылок на скачивание отчётов.

---

## [Release v3.5] — 2026-08-17 (PDF-ведомости диспетчера, завершение смен и многостраничный Excel)

### Добавлено
- **PDF-отчёты для диспетчера**: Кнопка «Отчёт PDF» в карточке каждого водителя в панели исполнения доставок, позволяющая мгновенно генерировать и скачивать готовую ведомость смены водителя без ручной пересылки файлов.
- **Информационный баннер завершения смены**: Автоматический блок подтверждения выполнения маршрута с быстрой ссылкой на скачивание итогового акта сдачи-приёмки и кассы.
- **Многостраничный Excel-отчёт**: Двухстраничная структура книги (`Сводка по рейсу` с KPI, кассой по типам оплат и итогами по водителям + `Детализация точек` с аудитом доставки и статусами).
- **Синхронизация закрытия смены в Telegram**: Уведомление водителю в Telegram отправляется автоматически при нажатии диспетчером кнопки «Завершить рейс».

### Изменено
- **Упрощение оплаты водителем**: Убран лишний ручной ввод денежных сумм, оставлен только выбор статуса и метода оплаты; добавлено предупреждение для заказов без суммы о необходимости сверки по накладной.
- **Кнопка выгрузки Excel**: Добавлена в шапку страницы результатов и в панель исполнения маршрутов.
...
| Файл | Назначение |
|------|-----------|
| `PILOT_AGREEMENT.md` | Договор на пилотное использование сервиса — для подписания с первым клиентом |
| `TERMS_OF_SERVICE.md` | Пользовательское соглашение — правила использования, запрещённые действия, отказ от гарантий точности маршрутов |
| `PRIVACY_POLICY.md` | Политика обработки персональных данных (ФЗ-152) — состав данных, цели, сроки хранения, удаление по запросу |
| `COMMERCIAL_OFFER.md` | Коммерческое предложение для дистрибьютора — проблема, возможности, расчёт ROI, формат пилота, прайс |
| `CLIENT_ONBOARDING.md` | Пошаговая инструкция для нового клиента — загрузка магазинов, создание маршрутов, печать, навигатор, WhatsApp |

### Контекст

Все документы написаны специально под реальный продукт SmartRoute (Railway + PostgreSQL, MVP, без авторизации).
Каждый документ содержит раздел юридического аудита «⚠️ Юридический аудит» с указанием слабых мест для проверки профессиональным юристом перед подписанием — этот раздел необходимо удалить перед передачей клиенту.

### Ключевые параметры ценообразования (зафиксированы в документах)

| Автопарк | Пилот (50%) | Стандарт |
|----------|------------|---------|
| до 3 машин | 2 500 руб/мес | 5 000 руб/мес |
| 4–8 машин | 4 500 руб/мес | 9 000 руб/мес |
| 9–15 машин | 7 500 руб/мес | 15 000 руб/мес |
| 16–30 машин | 11 000 руб/мес | 22 000 руб/мес |

---

## [Unreleased] — 2026-06-05 (Route sheet + WhatsApp fix)

### Добавлено

- **Маршрутный лист (печать)**: кнопка «🖨 Маршрутный лист» на странице результатов. При нажатии `window.print()` скрывает весь интерфейс и показывает чистые таблицы A4 по одной на каждую машину: №, Магазин, Адрес, Кол-во товара (пусто для заполнения), Прибытие, Отметка. Строки для подписей водителя и диспетчера.
- **Колонка «Кол-во товара»**: добавлена в маршрутный лист между «Адрес» и «Прибытие» для ручного заполнения кладовщиком/водителем.
- **`@page { size: A4 portrait; margin: 14mm 12mm }`** в `index.css` — точный формат A4 в диалоге печати браузера.

### Исправлено

- **WhatsApp отправлял только первый сегмент маршрута** (баг): `whatsapp_url()` принимал `yandex_url: str` (одна строка) и получал только `yurls[0]`. При маршрутах >19 точек остальные сегменты терялись.
  - Исправлено: `whatsapp_url(... yandex_urls: list)` — принимает весь список
  - 1 сегмент → `🗺 Навигатор: <url>` (формат не изменился)
  - 2+ сегмента → `🗺 Маршрут 1: <url>`, `🗺 Маршрут 2: <url>`, ...
  - Проверено для 10, 30, 50, 100 точек — ни одна ссылка не теряется

### Изменённые файлы

- `artifacts/smartroute/src/pages/result.tsx`
- `artifacts/smartroute/src/index.css`
- `artifacts/api-server/main.py`
