...
`parse_yandex_link(url)` извлекает `(lat, lon)` из ссылки Яндекс Карт.
Поддерживаемые форматы (в порядке проверки):

| Формат | Пример |
|--------|--------|
| `whatshere[point]=lon,lat` | `?whatshere[point]=37.617,55.755` |
| `ll=lon,lat` | `?ll=37.617,55.755` |
| `rtext=lat,lon~lat,lon` | `?rtext=55.755,37.617~55.74,37.60` |
| Короткие ссылки `/-/` | следует по редиректу |

Если ссылка распарсилась, но адрес не указан → вызывается `reverse_geocode_nominatim(lat, lon)`.

### Приоритеты определения координат в `create_store` и `import_stores`

```
1. Явно переданные lat + lon (точность ≥ 5 знаков)
2. Ссылка yandex_url → parse_yandex_link()
3. Геокодинг строки "{city} {address}"
```
