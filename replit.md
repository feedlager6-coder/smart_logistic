...
- **1С авто-создание магазинов**: `POST /api/v1/orders/batch` (и `/api/v1/webhooks/ingest`) принимают `auto_create_stores: bool = true`. При `true` незнакомые точки доставки создаются автоматически (geocode_status='pending', source='1c'). Приоритет матчинга: 1) `counterparty_code = stores.external_id` (точный), 2) fuzzy name match, 3) авто-создание. `counterparty_code` (в заказе) → `stores.external_id` (в магазине) — разные имена намеренно: `external_id` в `WebhookOrderItem` уже занят идемпотентностью заказа. `stores.external_id` и `stores.source` — новые колонки через миграцию. Ответ содержит `auto_created_stores: int`.
- **1С EPF архитектура**: SmartRoute раздаёт готовый `SmartRoute.epf` из `artifacts/integrations/1c/SmartRoute.epf`. Замена файла + `version.json` status="released" — без изменений кода. ZIP содержит EPF + BSL (Вариант Б) + Инструкция.txt. Директорская страница `/integrations` — без технических терминов. Страница для специалиста — `/integrations/1c/specialist`.
