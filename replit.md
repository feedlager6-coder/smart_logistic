...
- **Модель стоимости**: `cost_per_km` читается из `company_settings` (БД). Формула: `(fuel_price × consumption / 100) + (salary / 22 / 200)`. Дефолты: 67 ₽/л, 13 л/100 км, 55 000 ₽/мес → ~21.21 ₽/км. `ROAD_FACTOR = 1.4` — географическая константа (Haversine → реальный пробег), применяется ТОЛЬКО к монетарным метрикам. `saved_fuel_cost_rub` — экономия только топлива. `saved_rub_day` — полная экономия. Ответ `POST /api/route/build` → `savings` содержит `cost_per_km`, `fuel_price`, `fuel_consumption`, `driver_salary` (для расшифровки на странице результата). `route_sessions` хранит `cost_per_km` на момент построения.

## Product

- **Настройки компании**: страница `/settings` — цена топлива (₽/л), расход (л/100 км), зарплата водителя (₽/мес); live-калькулятор стоимости км; хранится в БД (`company_settings`); применяется при каждом построении маршрута; новые настройки не пересчитывают прошлые сессии — `cost_per_km` сохраняется исторически в `route_sessions`
