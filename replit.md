...
- **Яндекс.Навигатор лимит 20 точек**: мобильное приложение не строит маршрут при `rtext` > 20 точек. Константа `YANDEX_NAV_MAX_STOPS = 20`. Функция `yandex_nav_urls()` разбивает маршрут на сегменты ≤20. В ответе: `yandex_urls: list[str]` (новое) + `yandex_url: str` (backward compat = первый сегмент). Фронтенд показывает отдельные кнопки и amber-предупреждение при наличии нескольких сегментов. Экономика (km/savings) не меняется.
...
- **Модель стоимости**: `cost_per_km = 31 руб/км` (реальные дороги, Газель РФ 2026): топливо 7 + водитель с СВ 13 + ТО 7 + накладные 4 = 31 руб/км. `ROAD_FACTOR = 1.4` — коэффициент реальный пробег / Haversine; применяется ТОЛЬКО к монетарным метрикам (топливо, деньги), не к saved_km/saved_pct. `saved_fuel_cost_rub` — экономия только топлива. `saved_rub_day` — полная экономия. Оба поля в ответе `POST /api/route/build`. (Аудит 2026-06-03: saved_km и saved_pct честны — оба Haversine; rub_day скорректирован −13% от старого значения.)
