...
- **VRP**: Google OR-Tools — **capacitated sweep** (cap=⌈avg×1.5⌉, −15% km vs equal-angle) → TSP per sector → **inter-route Or-opt relocate** (пост-обработка, обычно −15-40% км); fallback на greedy если OR-Tools не установлен
...
- **Capacitated sweep**: Step 2 в `solve_vrp` использует `_cluster_by_capacitated_sweep` (cap=⌈avg×1.5⌉) вместо equal-angle `_cluster_by_sweep`. Когда сектор переполняется, излишние точки перетекают в следующий. Бенчмарк: −15.4% km (126.9 vs 149.9 km) при ratio 1.80x vs 3.22x на реальных данных (Махачкала, 120 точек/9 машин, depot на краю города). Equal-angle sweep коллапсирует если depot вне центроида точек.
