...
// ║   4. Разместите элементы формы (кнопки + поле даты):               ║
...
// ║   6. Заполните раздел НАСТРОЙКИ ниже                               ║
...
// ══════════════════════════════════════════════════════════════════
// НАСТРОЙКИ — заполните эти поля (URL и ключ уже встроены)
// ══════════════════════════════════════════════════════════════════
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
        // ── НАСТРОЙКИ — заполнены автоматически из SmartRoute ──────────
...
Функция ПроверитьСоединениеНаСервере()
        Если ПустаяСтрока(URLSmartRoute) Или URLSmartRoute = "{{BASE_URL}}" Тогда
                Возврат "⚙️ Не заполнен URLSmartRoute!" + Символы.ПС
                        + "Откройте Модуль формы и укажите адрес сервера SmartRoute.";
        КонецЕсли;

        Если ПустаяСтрока(APIКлюч) Или APIКлюч = "{{API_KEY}}" Тогда
        ...
        Если Ответ.КодСостояния = 200 Тогда
                // Дополнительно — узнаем количество магазинов в базе
                КолМаг = "?";
                Попытка
...
//   (или другое удобное время ДО начала работы диспетчера)
// ══════════════════════════════════════════════════════════════════

Процедура ОтправитьЗаявки() Экспорт
        // При вызове из регламентного задания отправляем заказы на СЕГОДНЯ
        ДатаДоставки = НачалоДня(ТекущаяДата());
        Рез = ОтправитьЗаявкиНаСервере(ДатаДоставки);
        // Записать результат в журнал регистрации для диагностики
...
Функция ОтправитьЗаявкиНаСервере(ДатаДоставки)
        ...
        // Адаптируйте запрос под вашу конфигурацию 1С:
        //  - ИмяДокументаЗаказа       — имя документа (напр. "ЗаказКлиента")
        //  - РеквизитДатыДоставки     — имя реквизита с датой доставки
        //  - РеквизитАдресаДоставки   — имя реквизита с адресом
        //  - РеквизитКоличества       — имя реквизита количества мест
        //  - РеквизитВеса             — имя реквизита веса в кг
        // ВАЖНО: Контрагент.Код — это внешний ID; его передача обязательна
        //        для точного совпадения магазина без ошибок!
        Запрос = Новый Запрос;
        Запрос.Текст =
        "ВЫБРАТЬ
        ...
        Если РезультатЗапроса.Количество() = 0 Тогда
                Возврат "ℹ️ Нет проведённых заказов на " + Формат(ДатаДоставки, "ДФ=дд.ММ.гггг") + "." + Символы.ПС
                        + "Убедитесь, что в реквизите «" + РеквизитДатыДоставки + "» документов" + Символы.ПС
                        + "стоит дата " + Формат(ДатаДоставки, "ДФ=дд.ММ.гггг") + " и документы проведены.";
        КонецЕсли;

        // ── 2. Сформировать массив заявок для отправки ──────────────────
        ...
        Для Каждого Строка Из РезультатЗапроса Цикл
                ...
                // ПОЛЯ ДЛЯ ТОЧНОГО СОВПАДЕНИЯ (передавайте всегда!)
                Заказ.Вставить("counterparty_code", СокрЛП(Строка(Строка.КодКонтрагента))); // Код контрагента 1С
        ...
        Тело.Вставить("replace_date",       Истина);  // заменить заявки за дату (идемпотентно)
        Тело.Вставить("auto_create_stores", Истина);  // создавать новые точки автоматически

        СтрокаJSON = _ЗаписьJSON(Тело);

        МаксПопыток = 3;
        ОжиданиеСек = 5; // пауза между попытками
        Соединение  = _ПолучитьСоединение(URLSmartRoute);
        Ответ       = Неопределено;
        ...
        Для НомерПопытки = 1 По МаксПопыток Цикл
                ...
                Исключение
                        Если НомерПопытки < МаксПопыток Тогда
                                ...
                                Пока ТекущаяДата() - ТекущееВремя < ОжиданиеСек Цикл
        ...
        ИначеЕсли Ответ.КодСостояния = 422 Тогда
                Возврат "❌ Ошибка в данных:" + Символы.ПС
                        + Лев(Ответ.ПолучитьТелоКакСтроку(), 500) + Символы.ПС + Символы.ПС
...
Функция _ПолучитьСоединение(URL)
        ...
        // Убираем путь — оставляем только хост
        ...
        Если HTTPS Тогда
                ...
                // (нужно для Let's Encrypt на старых версиях 1С и Windows Server 2012)
