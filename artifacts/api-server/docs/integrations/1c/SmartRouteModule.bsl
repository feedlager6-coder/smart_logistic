...
Функция ПроверитьСоединениеНаСервере()
	...
	Если ПустаяСтрока(Н.URL) Или Н.URL = "ВСТАВЬТЕ_АДРЕС_SMARTROUTE" Или Н.URL = "{{BASE_URL}}" Тогда
		Н.URL = "https://ais-dev-jfvktcabwhjb5vgwuetixe-145997301695.us-east5.run.app";
	КонецЕсли;

	Если ПустаяСтрока(Н.Ключ) Или Н.Ключ = "ВСТАВЬТЕ_API_КЛЮЧ" Или Н.Ключ = "{{API_KEY}}" Тогда
