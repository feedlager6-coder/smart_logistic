# SESSION_NOTES.md — Сессия 03.06.2026 (Яндекс.Навигатор + Аудит метрик экономии + Haversine vs OSRM)
...
## Аудит 2: Модель стоимости (cost_per_km)

**Задача:** исправить `cost_per_km` и добавить `ROAD_FACTOR` для монетарных расчётов.

### Изменения в `calculate_savings()`

| Параметр | Было | Стало | Причина |
|---------|------|-------|---------|
| `cost_per_km` | 50 руб/км | 31 руб/км | Разбивка: топливо 7 + водитель 13 + ТО 7 + накладные 4 |
| `ROAD_FACTOR` | (нет) | 1.4 | Haversine → реальный пробег; применяется к fuel_l, rub_day |
| `saved_fuel_l` | `saved_km × 0.1` | `saved_km × 1.4 × 0.1` | +40% (теперь честно) |
| `saved_rub_day` | `saved_km × 50` | `saved_km × 1.4 × 31` | −13% от старого |
| `saved_km`, `saved_pct` | Haversine | Haversine | Не менялись (корректны) |

---

## Аудит 3: Haversine vs OSRM для расчёта метрик экономии

**Задача:** проверить, улучшит ли переход на OSRM-расстояния точность показателей.
**Скрипт:** `artifacts/api-server/audit_osrm_vs_haversine.py`

### Методология

10 сценариев (Махачкала + Москва, 20–50 магазинов, 3–5 машин):
- Запуск `solve_vrp()` → оптимизированный порядок
- Вычисление km по Haversine (текущая модель)
- Вычисление km по OSRM Route API (реальные дороги) для тех же маршрутов
- Замер времени VRP + OSRM

### Ключевые результаты

| Сценарий | Hav saved% | OSRM saved% | Δ% | rf_opt | rf_base |
|----------|-----------|------------|-----|--------|--------|
| S02 Разбросанный (Мах 20) | 60% | 48% | −12 | 1.927 | 1.502 |
| S03 Трасса (20) | 55% | 47% | −8 | 1.396 | 1.165 |
| S04 Кластеры 4×5 | 62% | 62% | 0 | 1.412 | 1.415 |
| S06 Большой (Мах 50) | 69% | 62% | −7 | 1.806 | 1.451 |
| S07 Реал. Москва (30) | 35% | 31% | −4 | 1.575 | 1.482 |
| S08 Москва центр (20) | 66% | 59% | −7 | 1.935 | 1.603 |
| S10 Разброс (Мах 40) | 62% | 55% | −7 | 2.042 | 1.707 |
| **S01, S05, S09** | — | — | ⚠️ АНОМАЛИЯ | 32–117× | мусор OSRM |

**Аномалии (S01, S05, S09):** публичный OSRM не покрывает дороги Махачкалы — road_factor 32–117× вместо ожидаемых 1.3–1.6×. 3 из 10 сценариев (30%) дали полностью неверные данные.

### Статистика по валидным сценариям (n=7)

| Метрика | Значение |
|---------|---------|
| Среднее Δpct (OSRM − Hav) | **−6.4%** (ниже порога 15%) |
| Средний saved_km ratio | **1.308×** (наш ROAD_FACTOR=1.4 ≈ точен) |
| road_factor opt vs base | 1.728× vs 1.475× (близко → Hav честен) |
...
| Критерий | Порог | Факт | |
|---------|-------|------|--|
| Улучшение Δsaved_pct | ≥15% | 6.4% | ❌ |
| Overhead времени | ≤20% | 19% | ✅ |
| Надёжность OSRM | 0% аномалий | 30% аномалий | ❌ |

**РЕШЕНИЕ: ❌ НЕ ПЕРЕХОДИТЬ. Haversine + ROAD_FACTOR остаётся.**

**Обоснование цифрами:**
1. saved_pct меняется лишь на 6.4% → ниже порога 15%
2. Аномалии OSRM для Махачкалы (30% сценариев) недопустимы в production
3. ROAD_FACTOR=1.4 точен: средний OSRM ratio = 1.308 (ошибка ±10%)
4. road_factor opt ≈ road_factor base → Haversine корректно измеряет относительную экономию

### Изменения в коде

**Нет.** Решение — сохранить текущую модель. Скрипт аудита сохранён в
`artifacts/api-server/audit_osrm_vs_haversine.py` для повторного запуска в будущем.
